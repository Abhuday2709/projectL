...
export async function DELETE(request: NextRequest) {
    ...
    const docs = (await reviewClient.send(new QueryCommand({ TableName: DocumentConfig.tableName, KeyConditionExpression: 'chatId = :cid', ExpressionAttributeValues: { ':cid': sessionId } }))).Items || [];
    ...
    for (const doc of docs) {
        ...
        await reviewClient.send(new DeleteCommand({ TableName: DocumentConfig.tableName, Key: { chatId: sessionId, uploadedAt: doc.uploadedAt } }));
    }
